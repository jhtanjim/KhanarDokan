  ...
  return (
    <div className="flex bg-white rounded-2xl shadow-lg overflow-hidden mb-4">
      ...
      <div className="w-1/3 h-64 relative overflow-hidden">
        ...
        {/* Quantity Badge (if in cart) */}
        {itemQuantity > 0 && (
          <div className="absolute bottom-4 right-4 z-20 bg-green-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">
            {itemQuantity}
          </div>
        )}
      </div>
      ...
      <div className="w-2/3 p-6 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-center">
          <button
            onClick={() => handleAddToCart(item)}
            className={`flex items-center py-3 px-6 rounded-full text-base font-medium transition duration-300 ${
              isInCart(_id)
                ? "bg-green-500 text-white hover:bg-green-600"
                : "bg-primary text-white hover:bg-primary-dark"
            }`}
